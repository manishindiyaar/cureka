 ...
 *
 * Handles appointment booking, validation, and Cal.com integration
 */
...
export class AppointmentsController {
  /**
   * Create new appointment with online booking
   */
  static async createAppointment(req: Request, res: Response): Promise<Response> {
    try {
      // Validate request
      ...
      // Validate time is in future (minimum 1 hour ahead)
      ...
      const checkAvailability = await calComService.checkDoctorAvailability(
        doctor_id,
        requestedTime
      );
      ...
      const appointmentType = (appointment_type || AppointmentType.CONSULTATION) as keyof typeof APPOINTMENT_DURATIONS;
   ...
   */
  static async getAvailableSlots(req: Request, res: Response): Promise<Response> {
    try {
      ...
      const requestedDoctorId = doctor_id as string;
   ...
   */
  static async getAppointment(req: Request, res: Response): Promise<Response> {
    try {
   ...
   */
  static async updateAppointmentStatus(req: Request, res: Response): Promise<Response> {
    try {
      const { appointmentId } = req.params;
   ...
   * Cancel appointment
   */
  static async cancelAppointment(req: Request, res: Response): Promise<Response> {
    try {
      const { appointmentId } = req.params;
      // Authorization handled by authorizeAppointmentOwner middleware
